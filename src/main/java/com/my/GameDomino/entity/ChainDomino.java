...
public class ChainDomino implements Serializable {
    ...
    private List<ChainDominoHistory> listChainDominoHistory = new ArrayList<>();
    ...
    public String getStringListDomino() {
        return stringChainDomino;
    }

    public void setStringListDomino(String stringListDomino) {
        this.stringChainDomino = stringListDomino;
    }
    ...
    public ChainDominoHistory getMaxChain() {
        return listChainDominoHistory.stream()
                .max((s1, s2) -> ((Integer) s1.getStringChainHistory().length()).compareTo(s2.getStringChainHistory().length())).get();
    }
